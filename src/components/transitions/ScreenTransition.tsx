...
export function ScreenTransition() {
  ...
  useEffect(() => {
    console.log('[ScreenTransition] useEffect triggered:', {
      isTransitioning,
      transitionType,
      phoneScreenRect: phoneScreenRect
        ? {
    ...
    if (!isTransitioning || !transitionType || !phoneScreenRect || !overlayRef.current) {
      console.log('[ScreenTransition] exiting - conditions not met:', {
        isTransitioning,
        hasType: !!transitionType,
        hasRect: !!phoneScreenRect,
        hasRef: !!overlayRef.current,
      });
      return;
    }
  ...
  return (
    <div ref={overlayRef} className="pointer-events-none" aria-hidden="true">
      <div className="flex h-full w-full items-center justify-center">
        <div className="font-pixel text-lime text-xl">{config.text}</div>
      </div>
    </div>
  );
}
