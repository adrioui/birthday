...
export function RedeemSuccessModal({ isOpen, onClose }: RedeemSuccessModalProps) {
  ...
  useEffect(() => {
    ...
    tl.fromTo(
      ...
      { y: 0, opacity: 1, scale: 1, duration: prefersReduced ? 0 : 0.5, ease: 'back.out(1.7)' },
      '-=0.3'
    );
    ...
    tl.fromTo(
      messageRef.current,
      { y: 20, opacity: 0 },
      { y: 0, opacity: 1, duration: prefersReduced ? 0 : 0.4, ease: 'power2.out' },
      '-=0.2'
    );
  ...
  const handleClose = () => {
    ...
    tl.to(containerRef.current, {
      scale: prefersReduced ? 0 : 1.2,
      opacity: 0,
      rotation: prefersReduced ? 0 : 10,
      ...
      >
        ...
        <div className="text-center mb-8 relative">
          <div className="relative inline-block mb-4">
            ...
            <div ref={sparkle2Ref} className="absolute -bottom-2 -left-8 text-3xl">
              ✦
            </div>
            <div ref={sparkle3Ref} className="absolute top-1/2 -right-12 text-2xl">
              ⭐
            </div>
            <div ref={sparkle4Ref} className="absolute -top-4 left-1/2 -translate-x-1/2 text-xl">
              ✧
            </div>
          </div>
